...
import { Beef, Activity, Droplets, HeartPulse, LineChart, CheckCircle2, ChevronRight, ShieldCheck, Database } from "lucide-react";
...
export default function CattleManagement() {
  return (
    <div className="pt-20 pb-32">
      ...
      <section className="py-24 bg-white">
        <div className="section-container">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-32">
            ...
            <div className="bg-brand-background rounded-[40px] p-8 border border-brand-border shadow-inner aspect-square flex items-center justify-center">
               <div className="text-center">
                  <Beef className="w-32 h-32 text-brand-primary mx-auto mb-6 opacity-20" />
                  <p className="text-brand-muted font-bold uppercase tracking-widest text-[10px]">Visualizing Herd Performance</p>
               </div>
            </div>
          </div>
            ...
            {[
              {
                ...
                icon: Droplets,
                desc: "Monitor somatic cell count (SCC), milk fat components, and daily yield variance. Automated drying-off schedules and breeding alerts."
              },
              {
                title: "Health & Clinical",
                icon: HeartPulse,
                desc: "Full clinical EMR for every cow. Never miss a vaccination dose. Automated withdrawal period alerts for food safety compliance."
              },
              {
                title: "Fattening Logs",
                icon: LineChart,
                desc: "Track Feed Conversion Ratio (FCR) and Average Daily Gain (ADG). Optimize your yard entry and exit weights for maximum ROI."
              }
            ].map((card, i) => (
              <div key={i} className="p-10 bg-white border border-brand-border rounded-3xl hover:shadow-xl transition-all group">
          ...
          <div className="bg-brand-background rounded-[48px] p-12 md:p-20 overflow-hidden relative">
            ...
            <div className="absolute bottom-0 right-0 w-96 h-96 bg-white/40 rounded-full blur-3xl translate-x-1/2 translate-y-1/2"></div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 bg-white">
        <div className="section-container">
          <div className="bg-brand-primary rounded-[40px] p-12 md:p-20 text-center text-white shadow-2xl shadow-brand-primary/20">
            <h2 className="text-4xl md:text-5xl font-black mb-8 leading-tight">Ready to Digitize Your Farm?</h2>
            <p className="text-xl opacity-90 mb-12 max-w-2xl mx-auto">
              Join Pakistan's leading livestock enterprises. Start your free trial today.
            </p>
            <button className="bg-brand-navy text-white px-10 py-5 rounded-2xl font-bold hover:scale-105 transition-transform">
              Contact Sales Now
            </button>
          </div>
        </div>
